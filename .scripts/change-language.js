...
const noUnlink = args.filter((item) => item.toLowerCase() === '--no-unlink').length != 0;
...
const scripts = {
  javascript: {
    build: 'vrollup -c --environment NODE_ENV:production',
    ...
    'test:single': 'nyc --reporter=html --reporter=text --extension .js mocha --forbid-only',
  },
  typescript: {
    ...
    'test:single': 'nyc --reporter=html --reporter=text --extension .ts mocha --forbid-only',
  },
};
...
const config = async () => {
  switch (language.toLowerCase()) {
    case 'none':
      ...
      package.dependencies = removeKeys(package.dependencies, Object.getOwnPropertyNames(dependencies.javascript));
      package.dependencies = removeKeys(package.dependencies, Object.getOwnPropertyNames(dependencies.typescript));
      package.scripts = removeKeys(package.scripts, Object.getOwnPropertyNames(scripts.javascript));
      package.scripts = removeKeys(package.scripts, Object.getOwnPropertyNames(scripts.typescript));
    ...
    case 'typescript':
      ...
      break;
    default:
      console.error('No language selected. Please chose between: javascript and typescrypt');
      process.exit(1);
  }
